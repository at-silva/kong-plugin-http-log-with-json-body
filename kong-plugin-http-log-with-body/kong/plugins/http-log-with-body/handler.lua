...
function HttpLogHandler:access(conf)
  ...
  local is_json = str_find(content_type:lower(), "application/json", nil, true)
  if is_json then
...
function HttpLogHandler:log(conf)
  ...
  local ctx = kong.ctx.plugin
  local my_log = kong.log.serialize()
  if ctx.request_body then
    my_log.request.body = cjson.decode(ctx.request_body)
  end
  
  if ctx.response_body then
    my_log.response.body = cjson.decode(ctx.response_body)
  end

  local entry = cjson.encode(my_log)
